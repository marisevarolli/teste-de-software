describe('Transferência de usuário', () => {
...
describe('Depósito de usuário', () => {
    it('Deve permitir que o usuário acesse a aplicação, realize um depósito e faça logout', () => {
        ...
        cy.getByData('botao-enviar').click()
        cy.location('pathname').should('eq','/home')
        cy.getByData('select-opcoes').select('Depósito') /* no formulário, 'select-opcoes' é o campo onde selecionamos qual tipo de transação. Escolhemos 'Transferência' */
